      ...
      <section class="set-place">
        <div class="body">
          ...
          <div class="sidebar-form">
            ...
            <div class="form">
              <p class="form-name">نقشه</p>
              <p class="form-description">
                مکان دقیق اقامتگاه را در نقشه زیر انتخاب کنید تا علامت لوکیشن نمایان شود. توجه داشته باشید که لوکیشن اقامتگاه پس تکمیل هر رزرو برای میهمان ارسال خواهد شد، در نتیجه لازم است تا در انتخاب مکان اقامتگاه روی نقشه دقت کنید.
              </p>
              <form action="">
                <div id="map-wrap" style="height: 300px; margin-bottom: 20px;">
                  <client-only>
                    <l-map :zoom=13 :center="[this.cityLat,this.cityLng]" @click="addMarker">
                      <l-tile-layer url="http://{s}.tile.osm.org/{z}/{x}/{y}.png"></l-tile-layer>
                      <l-marker :lat-lng="[this.lat,this.lng]" v-if="lat !== 0"></l-marker>
                    </l-map>
                  </client-only>
                </div>
                <div class="row submit-cancel">
                  <button class="submit" type="button" v-if="updating === false" @click="updateLatLng">
                    بروزرسانی نقشه و مرحله بعد
                  </button>
                  <button class="submit" type="button" v-else>
                    چند لحظه صبر کنید
                  </button>
                  <nuxt-link class="cancel" :to="`/profile/residence/edit/parking/`+id">مرحله قبل</nuxt-link>
                </div>
              </form>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    async asyncData({params,$auth,$axios,redirect}) {
      ...
      const {data} = await $axios.post('api/v1/user/residence/editLocation',{id:params.id});
    ...
    methods: {
      ...
      async updateLatLng() {
        this.updating = true;
        const updateLocation = await this.$axios.post('api/v1/user/residence/updateLocation',{id:this.id,lat:this.lat,lng:this.lng});
        if(updateLocation.data.status === 1) {
          this.$snotify.success(updateLocation.data.msg, {timeout: 2000, showProgressBar: false, closeOnClick: true, pauseOnHover: true});
          this.$router.push('/profile/residence/edit/rules/'+this.id);
        } else if(updateLocation.data.status === 422) {
          this.errors = updateLocation.data.errors;
          this.$snotify.warning(updateLocation.data.msg, {timeout: 2000, showProgressBar: false, closeOnClick: true, pauseOnHover: true});
        } else {
          this.$snotify.error(updateLocation.data.msg, {timeout: 2000, showProgressBar: false, closeOnClick: true, pauseOnHover: true});
        }
        this.updating = false;
      },
      closeMenu() {
