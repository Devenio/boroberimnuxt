  ...
  <div>
    <main ref="main" class="main" :class="[vuexMenuState ? 'blur' : '',vuexLogregMobileState ? 'blur' : '']" @click="closeMenu">
      <section class="filters-places">
        <div class="title">
          <h1>اجاره {{ type.title }} در استان {{ province.name }}</h1>
          <hr>
          <div :class="[textshow ? `textshow` : `text`]" v-html="province.text"></div>
    ...
    <vk-modal center id="provinceModal" class="confirm-information-modal select-province-modal" :show.sync="provinceModal">
      ...
      <div class="form">
        <form @submit.prevent="">
          ...
          <p class="province-title" v-if="cities != null && !!provinceName">شهرها</p>
...
<script>
  ...
  export default {
    ...
    async asyncData({ route,params,$axios,redirect }) {
      let {data} = await $axios.post('api/v1/provinceResidences',{'slug':params.province,'type':params.type});
      if(data.status === 0)
        redirect('/404');
      return {
    ...
    methods: {
      ...
      async changeOrder(order) {
        ...
        const { data } = await this.$axios.post(`api/v1/provinceResidences`,this.form);
    ...
    head() {
      return {
        title: 'اجاره '+this.type.title+' در استان '+this.province.name,
        meta: [
          {
            hid: 'description',
            name: 'description',
            content: this.province.metaDesc
          },
        ],
      }
    }
