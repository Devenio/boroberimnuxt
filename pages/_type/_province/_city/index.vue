  ...
  <div>
    <main ref="main" class="main" :class="[vuexMenuState ? 'blur' : '',vuexLogregMobileState ? 'blur' : '']" @click="closeMenu">
      <section class="filters-places">
        <div class="title">
          <h1>اجاره {{ type.title }} در شهر {{ city.name }}</h1>
          <hr>
          <div :class="[textshow ? `textshow` : `text`]" v-html="city.text"></div>
...
<script>
  ...
  export default {
    ...
    async asyncData({ route,params,$axios,redirect }) {
      let {data} = await $axios.post('api/v1/cityResidences',{'slug':params.city,'province':params.province,'type':params.type});
      if(data.status === 0)
        redirect('/404');
      return {
    ...
    methods: {
      ...
      async changeOrder(order) {
        ...
        const { data } = await this.$axios.post(`api/v1/cityResidences`,this.form);
    ...
    head() {
      return {
        title: 'اجاره '+this.type.title+' در شهر '+this.city.name,
        meta: [
          {
            hid: 'description',
            name: 'description',
            content: this.city.metaDesc
          },
        ],
      }
    }
