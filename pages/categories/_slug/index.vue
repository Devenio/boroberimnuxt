  ...
  <div>
    <main ref="main" class="main" :class="[vuexMenuState ? 'blur' : '',vuexLogregMobileState ? 'blur' : '']" @click="closeMenu">
      <section class="filters-places">
        <div class="title">
          <h1>اجاره اقامتگاه‌های {{ category.title }}</h1>
          <hr>
          <div :class="[textshow ? `textshow` : `text`]" v-html="category.text"></div>
...
<script>
  ...
  export default {
    ...
    async asyncData({ route,params,$axios,redirect }) {
      let {data} = await $axios.post('api/v1/categoryResidences',{'slug':params.slug});
      if(data.status === 0)
        redirect('/404');
      return {
    ...
    methods: {
      ...
      async changeOrder(order) {
        ...
        const { data } = await this.$axios.post(`api/v1/categoryResidences`,this.form);
    ...
    head() {
      return {
        title: 'اجاره اقامتگاه های '+this.category.title,
        meta: [
          {
            hid: 'description',
            name: 'description',
