...
<script>
  ...
  export default {
    ...
    async asyncData({$axios,route}) {
      const { data } = await $axios.post(`api/v1/specialResidences`,{title:route.query.title,
    ...
    methods: {
      ...
      async changeOrder(order) {
        ...
        const { data } = await this.$axios.post(`api/v1/specialResidences`,this.form);
    ...
    head() {
      return {
        title: 'اقامتگاه های خاص - '+this.settings.sitename,
        meta: [
          {
            hid: 'description',
            name: 'description',
