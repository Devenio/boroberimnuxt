  ...
  <div>
    <main ref="main" class="main" :class="[vuexMenuState ? 'blur' : '',vuexLogregMobileState ? 'blur' : '']" @click="closeMenu">
      <section class="filters-places">
        <div class="title">
          <h1>اقامتگاه‌هایی با قابلیت رزرو فوری</h1>
          <hr>
...
<script>
  ...
  export default {
    ...
    async asyncData({$axios,route}) {
      const { data } = await $axios.post(`api/v1/fastResidences`,{title:route.query.title,
    ...
    methods: {
      ...
      async changeOrder(order) {
        ...
        const { data } = await this.$axios.post(`api/v1/fastResidences`,this.form);
    ...
    head() {
      return {
        title: 'اقامتگاه های تحویل فوری - '+this.settings.sitename,
        meta: [
          {
            hid: 'description',
            name: 'description',
