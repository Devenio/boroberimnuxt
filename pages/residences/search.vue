  ...
  <div>
    <main ref="main" class="main" :class="[vuexMenuState ? 'blur' : '',vuexLogregMobileState ? 'blur' : '']" @click="closeMenu">
      <section class="filters-places">
        <div class="title">
          <h1>نتایج جستجوی شما</h1>
          <hr>
          <p><span>{{ total }}</span> مورد پیدا شد</p>
        </div>
        <div class="kinds">
        ...
        <form action="">
          ...
          <div class="city-lists" v-if="cities != null">
            <a href="javascript:void(0)" v-bind:class="[citiesSearchNames.includes(city.name) ? 'active' : '']"
               v-for="city in cities" @click="addCity(city)">
                  <span>
                    <i class="bx bx-notification"></i>
                    {{ city.name }}
                  </span>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        types: [
          { 
            title: "مقصد‌اقامتگاه",
            onClick: this.showModalProvince,
            icon: "bxs-circle dot"
          }
        ]
      }
    ...
    async asyncData({$axios,route}) {
      const { data } = await $axios.post(`api/v1/citiesResidences`,{title:route.query.title,
    ...
    methods: {
      ...
      async changeOrder(order) {
        ...
        const { data } = await this.$axios.post(`api/v1/residences`,this.form);
    ...
    watch: {
      $route (to, from) {
        ...
        this.form.title = to.query.title;
        this.form.startPerson = to.query.startPerson;
        this.form.province = to.query.province;
        this.form.provinceName = to.query.provinceName;
        this.form.city = to.query.city;
        this.search();
      },
      date: {
        deep: true,
    ...
    head() {
      return {
        title: 'جستجو - '+this.settings.sitename,
        meta: [
          {
            hid: 'description',
            name: 'description',
