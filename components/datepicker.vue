        ...
        <div class="grid-cols-7 grid w-full">
          ...
          >
            <button
              ...
              "
              :disabled="
                ((!shouldApplyRangeLimit && isDisabled(day)) ||
                  !isSelectable(day) ||
                  (shouldApplyRangeLimit && !isInrangeOfUnselectable(day))) &&
                !enableAllDays
              "
              :class="[
                isHoliday(day) && !isInrange(day).value && !isSelected(day)
                  ? 'dp-text-red-400'
                  : '',
                ((!shouldApplyRangeLimit && isDisabled(day)) ||
                  !isSelectable(day) ||
            ...
            >
              ...
              >
                ...
                <span class="text-sm font-normal" v-if="size !== 'sm'">
                  {{ getPrice(day) }}
                </span>
                <span
...
<script>
  ...
  export default defineComponent({
    ...
    mounted() {
      ...
      this.daysMap = this.mapArr(this.days)
      if (this.type === 'range') {
    ...
    methods: {
      ...
      inp(event) {
        // console.log(event, event.target?.attributes?.['data-day']?.value)
        this.inpday =
      ...
      mapArr(arr) {
        ...
        if (model) {
          const map = {}
          for (let i = 0; i < model.length; i++) {
            const { year, month, date } = this.str2date(model[i].date)
